...
def main():
    ...
    train_loader = datalaoder.load_cifar_data(cifar10_path, category_1, category_2, args.imbalance_rate,  args.corruption_type,
                                                 args.corruption_level,  args.batch_size)

    test_loader, val_loader= datalaoder.load_cifarC_data(cifar10c_path, category_1, category_2, args.imbalance_rate,  args.corruption_type,
                                                 args.corruption_level,  args.batch_size)
    ...
    for itr, (test_img, test_label) in enumerate(test_loader):
        ...
        probs = torch.sigmoid(out_test)
        probs = probs[:, 0].cpu().detach().numpy()
        labels = test_label.cpu().detach().numpy()
        print("the probs of testdata: ", probs)

        if len(set(labels)) > 1:  # 如果存在多个类别
            auc = roc_auc_score(labels, probs)
            print(f"the auc of {itr} batch is : ", auc)
            test_auc_tmp.append(auc)
        else:
            print(f"Skipping batch {itr} due to single class in y_true")
    ...
    test_auc_mean = np.mean(test_auc_tmp)
    test_accuracy_mean = np.mean(test_acc_tmp)

    print("test auc mean is", test_auc_mean)
